...
public class KMTestUtils {
  ...
  public static short getDeviceInfoExp() {
    ...
    short map = KMMap.instance((short) 14);
    // Canonical order is hard-coded.
    //brand
    KMMap.cast(map).add((short) 0, textStrExp, textStrExp);
    // model
    KMMap.cast(map).add((short) 1, textStrExp, textStrExp);
    //product
    KMMap.cast(map).add((short) 2, textStrExp, textStrExp);
    // device info version
    KMMap.cast(map).add((short) 3, textStrExp, intExp);
    // vb state
    KMMap.cast(map).add((short) 4, textStrExp, textStrExp);
    // osVersion
    KMMap.cast(map).add((short) 5, textStrExp, textStrExp);
    // attest id state
    KMMap.cast(map).add((short) 6, textStrExp, textStrExp);
    //manufacturer
    KMMap.cast(map).add((short) 7, textStrExp, textStrExp);
    // verified boot hash
    KMMap.cast(map).add((short) 8, textStrExp, byteBlobExp);
    // security level
    KMMap.cast(map).add((short) 9, textStrExp, textStrExp);
    // boot patch level
    KMMap.cast(map).add((short) 10, textStrExp, intExp);
    // bootloader state
    KMMap.cast(map).add((short) 11, textStrExp, textStrExp);
    // system patch level
    KMMap.cast(map).add((short) 12, textStrExp, intExp);
    // vendor patch level
  ...
  public static void validateProtectedData(KMSEProvider cryptoProvider, KMEncoder encoder,
      KMDecoder decoder,
      byte[] EEK_KEY_ID, KeyPair eekKey, byte[] CsrChallenge, byte[] encodedCoseKeysArray,
      boolean testMode, short protectedDataArrPtr,
      short deviceInfoMapPtr,
    ...
    short ephemeralPubLen = getSenderPublicKeyAndKeyIdFromRecipientStructure(decoder, EEK_KEY_ID,
        protectedDataArrPtr,
        ephemeralPub, (short) 0,
    ...
    short sessionKeyLen =
        ecdhHkdfDeriveKey(cryptoProvider, encoder, eekPriv, (short) 0, eekPrivLen, eekPub,
            (short) 0, eekPubLen,
            ephemeralPub, (short) 0,
    ...
    params =
        decoder.decode(KMCoseHeaders.exp(), KMByteBlob.cast(params).getBuffer(),
            KMByteBlob.cast(params).getStartOff(),
    ...
    short authDataLen = encoder.encode(coseEncryptStr, authData, (short) 0, (short) 256);
    short cipherText = KMArray.cast(protectedDataArrPtr).get((short) 2);
    byte[] authTag = new byte[AES_GCM_AUTH_TAG_LENGTH];
    short encryptedDataLen = (short) (KMByteBlob.cast(cipherText).length()
    ...
    Util.arrayCopyNonAtomic(KMByteBlob.cast(cipherText).getBuffer(),
        KMByteBlob.cast(cipherText).getStartOff(),
    ...
    //--------------------------------------------
    validateSignedMac(cryptoProvider, encoder, decoder, CsrChallenge, encodedCoseKeysArray,
        KMArray.cast(payloadPtr).get((short) 0), pub, (short) 0, pubLen,
        deviceInfoMapPtr,
        pubKeysToSignMac);
    //--------------------------------------------
