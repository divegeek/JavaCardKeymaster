...
public class RemotelyProvisionedComponentDevice {
    ...
    if (authorizedEekRoots == null) {
      authorizedEekRoots =
          new Object[]
              {
                  new byte[]{
                      0x5c, (byte) 0xea, 0x4b, (byte) 0xd2, 0x31, 0x27, 0x15, 0x5e, 0x62,
                      (byte) 0x94, 0x70,
                      0x53, (byte) 0x94, 0x43, 0x0f, (byte) 0x9a, (byte) 0x89, (byte) 0xd5,
                      (byte) 0xc5, 0x0f,
                      (byte) 0x82, (byte) 0x9b, (byte) 0xcd, 0x10, (byte) 0xe0, 0x79, (byte) 0xef,
                      (byte) 0xf3,
  ...
  private short validateAndExtractPublicKey(short coseMacPtr, byte[] scratchPad) {
    ...
    short hmacLen = seProvider.hmacSign(KMByteBlob.cast(macKey).getBuffer(),
        KMByteBlob.cast(macKey).getStartOff(),
  ...
  private short constructCoseMacForRkpKey(boolean testMode, byte[] scratchPad, short pubKey) {
    ...
    short hmacLen = seProvider
        .hmacSign(KMByteBlob.cast(macKey).getBuffer(), KMByteBlob.cast(macKey).getStartOff(),
