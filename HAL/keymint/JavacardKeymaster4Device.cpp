...
static inline std::unique_ptr<se_transport::TransportFactory>& getTransportFactoryInstance() {
    if (pTransportFactory == nullptr) {
        pTransportFactory = std::unique_ptr<se_transport::TransportFactory>(
            new se_transport::TransportFactory(true));
...
decodeData(CborConverter& cb, const std::vector<uint8_t>& response, bool hasErrorCode) {
    auto [item, errorCode] = cb.decodeData<T>(response, hasErrorCode);
    if (T::OK != errorCode) errorCode = translateExtendedErrorsToHalErrors<T>(errorCode);
...
static bool isSEProvisioned() {
    ...
    std::tie(item, errorCode) =
        cborConverter.decodeData(std::vector<uint8_t>(response.begin(), response.end() - 2), true);
    if (item != NULL) {
        uint64_t status;
...
Return<void> JavacardKeymaster4Device::getHardwareInfo(getHardwareInfo_cb _hidl_cb) {
    ...
    if (ret == ErrorCode::OK) {
        ...
        std::tie(item, ret) =
            cborConverter_.decodeData(std::vector<uint8_t>(resp.begin(), resp.end() - 2), false);
        if (item != nullptr) {
...
JavacardKeymaster4Device::getHmacSharingParameters(getHmacSharingParameters_cb _hidl_cb) {
    ...
    if (ErrorCode::OK == errorCode) {
        ...
        std::tie(item, errorCode) = decodeData(
            cborConverter_, std::vector<uint8_t>(cborData.begin(), cborData.end() - 2), true);
        if (item != nullptr) {
...
#ifdef VTS_EMULATOR
    /* TODO temporary fix: vold daemon calls performHmacKeyAgreement. At that time when vold calls
     * this API there is no network connectivity and socket cannot be connected. So as a hack we are
     * calling softkeymaster to getHmacSharing parameters.
     */
    else {
        ...
        static_assert(sizeof(response.params.nonce) == hmacSharingParameters.nonce.size(),
                      "Nonce sizes don't match");
        memcpy(hmacSharingParameters.nonce.data(), response.params.nonce,
               hmacSharingParameters.nonce.size());
...
JavacardKeymaster4Device::computeSharedHmac(const hidl_vec<HmacSharingParameters>& params,
    ...
    /* TODO temporary fix: vold daemon calls performHmacKeyAgreement. At that time when vold calls
     * this API there is no network connectivity and socket cannot be connected. So as a hack we are
     * calling softkeymaster to computeSharedHmac.
     */
    else {
...
JavacardKeymaster4Device::deviceLocked(bool passwordOnly,
    ...
    std::unique_ptr<Item> item;
    std::vector<uint8_t> cborOutData;
    ::android::hardware::keymaster::V4_1::ErrorCode errorCode =
