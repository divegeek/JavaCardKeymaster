...
static ErrorCode generateOperationHandle(uint64_t& oprHandle) {
    ...
    //Check if operationHandle is present in operationTable
    uint64_t cnt = 1;
    uint64_t mask;
    for(; cnt < MAX_COUNTER_VALUE; cnt++) {
        ...
        if (it == operationTable.end()) {
            //No Strongbox Operation found with this operation handle.
            //Check for Software operation.
    ...
    if (cnt == MAX_COUNTER_VALUE) {
        LOG(ERROR) << "operation handle count reached max operations";
...
static ErrorCode createOprHandleEntry(uint64_t origOprHandle, uint64_t mask/* SW or HW */, uint64_t& newOperationHandle) {
    ErrorCode errorCode = ErrorCode::OK;
    if (ErrorCode::OK != (errorCode = generateOperationHandle(newOperationHandle))) {
        return errorCode;
    }
    //mask the operationhandle
    newOperationHandle |= (mask << 56);
    operationTable[newOperationHandle] = origOprHandle;
    return errorCode;
}
...
static ErrorCode getOrigOperationHandle(uint64_t halGeneratedOperationHandle, uint64_t& origOprHandle) {
    std::map<uint64_t, uint64_t>::iterator it = operationTable.find(halGeneratedOperationHandle);
    if (it == operationTable.end()) {
...
static bool isStrongboxOperation(uint64_t halGeneratedOperationHandle) {
    return (SB_KM_OPR == (halGeneratedOperationHandle >> 56));
}
